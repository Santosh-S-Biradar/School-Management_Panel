...
const ParentAttendancePage = () => {
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => {
        if (data?.[0]) {
          setChildName(data[0].name || 'Your Child');
          api
            .get(`/parent/attendance/${data[0].id}`)
  ...
  return (
    <AppLayout>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
        <div className="text-xs font-semibold uppercase tracking-wide text-ink-500">{childName} Attendance Percentage</div>
      ...
      <div className="mt-6">
        {records.length === 0 ? (
          <EmptyState
            ...
            description="Attendance logs will appear once marked by teachers."
          />
        ) : (
