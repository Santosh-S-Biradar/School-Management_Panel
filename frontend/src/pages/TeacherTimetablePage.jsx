...
const TeacherTimetableGrid = ({ timetable }) => {
  const byCell = new Map();
  timetable.forEach((item) => {
    byCell.set(buildKey(item.day_of_week, normalizeTime(item.start_time)), item);
  });

  const slots = buildSlotsFromEntries(timetable);
                    ...
                    {entry ? (
                      entry.entry_type === 'break' ? (
                        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 shadow-sm">
                          ...
                          <div className="mt-1 text-xs text-amber-800">{entry.class_name} | {entry.section_name || 'All Sections'}</div>
                          <div className="text-xs text-amber-700">Room {entry.room || '-'}</div>
                        </div>
                      ) : (
                        <div className="rounded-xl border border-brand-200 bg-gradient-to-br from-brand-50 to-white p-3 shadow-sm">
                          ...
                          <div className="mt-1 text-xs text-ink-700">{entry.class_name} | {entry.section_name || 'All Sections'}</div>
                          <div className="text-xs text-ink-500">Room {entry.room || '-'}</div>
                        </div>
                      )
                    ) : (
                      <div className="h-[82px] rounded-xl border border-dashed border-slate-200 bg-slate-50" />
                    )}
                  </td>
                );
              })}
...
const TeacherTimetablePage = () => {
  ...
  useEffect(() => {
    api.get('/teacher/timetable').then(({ data }) => setTimetable(data || [])).catch(() => {});
  }, []);

  return (
    <AppLayout>
      <PageHeader title="Timetable" subtitle="Your class teaching schedule." />
      <div className="mt-6">
        {timetable.length === 0 ? (
          <EmptyState title="No timetable" description="Your schedule will appear after class periods are assigned." />
        ) : (
